...
static void set_axis_is_at_home(const AxisEnum axis) {
   ...
   */
  if (axis == Z_AXIS) {
    current_position[axis] -= suppl_zoffset - Z_PROBE_OFFSET_FROM_EXTRUDER;
...
void clean_up_after_endstop_or_probe_move() {
  ...
  inline void do_probe_raise(const float z_raise) {
    ...
    if (Z_PROBE_OFFSET_FROM_EXTRUDER < 0) z_dest -= Z_PROBE_OFFSET_FROM_EXTRUDER;
  ...
  float probe_pt(const float &rx, const float &ry, const bool stow, const uint8_t verbose_level, const bool probe_relative=true) {
    ...
    if (!DEPLOY_PROBE()) {
      measured_z = run_z_probe() + Z_PROBE_OFFSET_FROM_EXTRUDER;
...
inline void gcode_M226() {
  ...
  inline void gcode_M280() {
    ...
    FORCE_INLINE void mod_suppl_zoffset(const float &offs) {
      suppl_zoffset += offs;
      SERIAL_ECHO_START();
      SERIAL_ECHOLNPAIR(MSG_PROBE_Z_OFFSET ": ", suppl_zoffset);
    }
  #endif
